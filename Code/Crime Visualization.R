...
library("scales")
library(fmsb)
setwd("/Users/emil/Desktop/Emil_datascience")
...
# drug offence rate and crime count for 2022
...
ggplot(data = crime_dataset_drugs, aes(x = District, y = `Drug Offence Rate`, fill = `Falls within`)) + #setting x-axis and y-axis values
  ...
  labs(title = "Box Plot of Drug Offence Rate By District of 2022") +
  coord_flip() 
  
#      2022 -March Vehicle Crime Rate Per 10000 people Radar Chart        #
...
crime_dataset_vehicle <- cleaned_crime_dataset %>% 
  ...
  na.omit() %>% 
  filter(`Crime type` == "Vehicle crime" & `Date of crime` == "2022-03") %>%   
...
radar_data <- crime_dataset_vehicle %>%
  select(District, `Vehicle Crime Rate`, `Falls within`, `Crime Count`, `Crime type`) %>%
  unique()  # Assuming you want unique districts

# Find the max values for scaling the radar chart
max_values <- colMaxs(radar_data[, -1], na.rm = TRUE)

# Create a dataframe with max values
max_row <- data.frame(District = "Max", max_values)

# Add the max_row dataframe to the start of radar_data
radar_data <- rbind(max_row, radar_data)

# Load libraries for radar chart
library(fmsb)
...
crime_dataset_robbery <-cleaned_crime_dataset %>% 
  ...
  select(`Short Postcode`,`Crime type`,`Crime Count`, `Population`, `Date of crime`, `Falls within`, District) %>% #select the required columns
  na.omit() %>% 
  filter(`Crime type`== "Robbery" & `Date of crime`=="2022-03") %>%   #filtering to show only vehicle crimes of 2022 -March
...
# Creating a custom color palette for the pie chart
custom_colors <- c("#FF9999", "#66B2FF", "#99FF99", "#FFCC99", "#c2c2f0", "#ffb3e6", "#b3e0ff", "#ff6666", "#c2f0c2")
...
ggplot(crime_dataset_robbery, aes(x = "", y = TotalRobberyCrimeRate, fill = District)) + #defining x axis and y axis values
  ...
  theme_void() +
  labs(fill = "Districts", title = "Robbery Crime Rate by District in March 2022") #defining lables



#-----------2019-2022 Drug Offence Rate In Kent and Surrey Line Graph-----------#
...
#creating line graph of average drug offence rate from 2019 to 2022 in Kent and Surrey 
Grouped_drug_crime %>%
  ...
  labs(title = "Line Graph of the Drug Offence Rate from 2019 to 2022", #defining labels 
       x = "Year",
